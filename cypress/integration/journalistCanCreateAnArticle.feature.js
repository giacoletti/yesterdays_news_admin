  ...
  describe('can be created by a journalist', () => {
    before(() => {
      ...
      cy.intercept("GET", "/api/auth/validate_token", {
        fixture: 'authenticated_journalist_response.json',
        headers: { uid: 'thomas@mail.com', token: '1234567890' }
      ...
      cy.get('[data-cy=email-field]').type('thomas@mail.com');
      cy.get('[data-cy=password-field]').type('password');
      cy.get('[data-cy=login-button]').click();

      cy.get('[data-cy=flash-message]').should('contain.text', 'Welcome Thomas Ochman');
      
      // let's create an article by clicking a CTA: "Create new article"
